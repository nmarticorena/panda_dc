...
def replace_gripper(
    robot: rtb.models.Panda, env: swift.Swift
):  # TODO: Improve for other grippers, at the moment is just the soft gripper
    ...
    left_finger = sg.Mesh(
        gripper_path,
        pose=sm.SE3(0.013, 0, 0.015) @ sm.SE3.Rz(90, unit="deg") @ sm.SE3.Rx(180, unit="deg"),
    )
    left_finger.color = [0.0, 1.0, 0, alpha]
    right_finger = sg.Mesh(
        gripper_path,
        pose=sm.SE3(-0.013, 0, 0.015) @ sm.SE3.Rz(-90, unit="deg") @ sm.SE3.Rx(180, unit="deg"),
    )
    right_finger.color = [0.0, 1.0, 0, alpha]
